...
function g(filename; hurst = collect(0.3:0.1:0.9))
    ...
    shapes = [:xcross, :circle, :utriangle,:square,:dtriangle, :diamond, :cross]
    ...
    p1 = plot(size=(400,300),
              ...
              fg_legend=:white, 
              dpi=150, frame=:box,
              xaxis=("Re", [1.2,700], [1,10,100,1000], font(11), :log),
              yaxis=(L"G", [11.35,1000], [10,100,1000], font(11), :log),
              grid=false)
    
#               xaxis=(L"\log_{10}\left(Re \right)", [1.2,700], [1,10,100,1000], font(11), :log),
#               yaxis=(L"\log_{10}\left(G \right)", [11.35,1000], [10,100,1000], font(11), :log),
    
    @. model(x,p) = p[1]+p[2]*x+p[3]*x^2 # model to fit G
  
    coefs = DataFrame(h=Float64[], α=Float64[], β=Float64[], γ=Float64[])
    for (i,h) in enumerate(hurst)
        ...
        p1 = plot!(xfit, yfit, linewidth=1, color=cl[i], label="")
...
p1
# savefig(p1, "../../paper/v7/fig2.pdf")

function collapse(filename; hurst = collect(0.3:0.1:0.9), plt=false, δs=0.15:0.05:0.25)
    """
    ...
    p1 = plot(size=(400,300),
              ...
              fg_legend=:white, 
              dpi=150, frame=:box,
              xaxis=(L"\mathrm{Re}/H", [1.2,3000], [1,10,100,1000], font(11), :log),
              yaxis=(L"G/\alpha w^2", [1., 10], [1,10^0.5,10],font(11),:log),
              # xaxis=(L"\log_{10}\left, (Re/H\right)", font(11)),
              # yaxis=(L"\log_{10}\left(G/\alpha w^2\right)", font(11)),
        grid=false)
...
# pyplot()
# x,y = collapse("data/G-all_1.csv", plt=false, δs=0.025:0.005:0.04)
# p1 = collapse("data/G-all_1.csv", plt=true, δs=0.025)
# savefig("../../paper/v7/fig4.pdf")
...
function plotalphaH(coefs)
    ...
    x = coefs.h
    y = coefs.α
    p4 = plot(size=(400,300),
                  legendfontsize=9, legend=:inside,fg_legend=:lightgray, 
                  dpi=150, frame=:box, grid=false,
                  xaxis=(L"H",font(16), [0.3,0.9]), yaxis=(L"α w^2", [11.65,68.35],font(16)),
                  background_color_outside=:transparent,
                  background_color_inside=:transparent,
                  background_color_legend=:transparent,)
    p4 = plot!(x,y, markershape=:circle, linewidth=0.0, markersize=9, label="")
    
    
    plot!([12], seriestype = :hline, linewidth=1, color=:red, linestyle = :dash, label=L"α w^2=12")
...
pα= plotalphaH(coefs)
pα
savefig("../../paper/v7/fig3_inset.pdf")
...
#     x = df.Lp
    ...
    p4 = plot(size=(400,300),
                  legendfontsize=9, legend=:inside,fg_legend=:lightgray, 
                  dpi=150, frame=:box, grid=false,
                  xaxis=(L"τ", font(11)), yaxis=(L"α w^2", font(11)),
                  background_color_outside=:transparent,
                  background_color_inside=:transparent,
                  background_color_legend=:transparent,)
    p4 = plot!(x,y, markershape=:circle, linewidth=0., markersize=8, label="")
    @. model(x,p) = p[1].+p[2].*x
...
function plotbeta(coefs)
    ...
    p4 = plot(size=(400,300),
                  legendfontsize=9, legend=:inside,fg_legend=:lightgray, 
                  dpi=150, frame=:box, grid=false,
                  xaxis=(L"α w^2/H", font(11)), yaxis=(L"\beta w", font(11)),
                  background_color_outside=:transparent,
                  background_color_inside=:transparent,
                  background_color_legend=:transparent,)
    p4 = scatter!(x,y, markershape=:circle, markersize=8, linewidth=0.5, label="")
    p0 = [0.02, 0.0]
    lb = [0.0, -0.1]
    ub = [0.1, 0.0]
...
function plotgamma(coefs)
    ...
    p4 = plot(size=(400,300),
                  legendfontsize=9, legend=:inside,fg_legend=:lightgray, 
                  dpi=150, frame=:box, grid=false,
                  xaxis=(L"α w^2/H^2", font(13)), yaxis=(L"\gamma", [0,2.3e-4], 0:1e-4:3e-4, font(13)),
                  #background_color_outside=:transparent,
                  #background_color_inside=:transparent,
    )
    
    p4 = plot!(x,y, markershape=:utriangle, markersize=9, color=2, linewidth=0.8, label="")
    @. model(x,p)=p[1]*x+p[2]
    fit = curve_fit(model, x[end-4:end],y[end-4:end],[1.0,0.0], lower=[0.0, -0.1], upper=[Inf, 0.0])
    xfit = 0:240
    yfit = model(xfit,fit.param)
